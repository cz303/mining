...
class Tickets extends Client_Controller {
    ...
    public function __construct() {
        parent::__construct();		$this->load->helper('download');
    ...
    public function index($Vrrb21ym0qp1 = NULL, $Vwfsll4zanwn = NULL) {
        ...
        $Vou4vxorrdoe['breadcrumbs'] = lang('tickets');
        $Vou4vxorrdoe['sub'] = 5;
        ...
        if ($Vrrb21ym0qp1 == 'tickets_details') {
            ...
            if ($Vou4vxorrdoe['tickets_info']->upload_path) {
                ...
                redirect('client/tickets/index/tickets_details/' . $Vwfsll4zanwn);
            }
        ...
        $Vou4vxorrdoe['all_tickets_info'] = $this->tickets_model->get_by(array('reporter' => $this->session->userdata('user_id')), FALSE);
        ...
        $Vou4vxorrdoe['subview'] = $this->load->view('client/tickets/' . $Vgvgl531vsfk, $Vou4vxorrdoe, TRUE);
        $this->load->view('client/_layout_main', $Vou4vxorrdoe); 
    }

    public function create_tickets($Vwfsll4zanwn = NULL) {
        $Vou4vxorrdoe = $this->tickets_model->array_from_post(array('ticket_code', 'subject', 'priority', 'departments_id', 'body', 'bill_no'));
        $Vou4vxorrdoe['status'] = 'open';
        $Vou4vxorrdoe['reporter'] = $this->session->userdata('user_id');
        ...
        if (!empty($_FILES['upload_file']['name'])) {
            ...
            $Vwk2nao2d33x == TRUE || redirect('client/tickets');
    ...
    public function change_status($Vwfsll4zanwn, $Va5zq3yanff3) {
        ...
        $Vou4vxorrdoe['modal_subview'] = $this->load->view('client/tickets/_modal_change_status', $Vou4vxorrdoe, FALSE);
    ...
    public function answered() {
        ...
        $this->tickets_model->_table_name = 'tbl_tickets';
        $this->tickets_model->_order_by = 'tickets_id';
        $Vou4vxorrdoe['all_tickets_info'] = $this->tickets_model->get_by(array('status' => 'answered', 'reporter' => $this->session->userdata('user_id')), FALSE);

        $Vpxamdyeznwr = $this->session->userdata('user_id');
        $Vsqvwshxevqn = $this->tickets_model->check_by(array('user_id' => $Vpxamdyeznwr), 'tbl_users');
        $Vou4vxorrdoe['role'] = $Vsqvwshxevqn->role_id;

        $Vou4vxorrdoe['subview'] = $this->load->view('client/tickets/tickets', $Vou4vxorrdoe, TRUE);
        $this->load->view('client/_layout_main', $Vou4vxorrdoe); 
    }
    ...
    public function closed() {
        ...
        $this->tickets_model->_table_name = 'tbl_tickets';
        $this->tickets_model->_order_by = 'tickets_id';
        $Vou4vxorrdoe['all_tickets_info'] = $this->tickets_model->get_by(array('status' => 'closed', 'reporter' => $this->session->userdata('user_id')), FALSE);
    ...
    public function open() {
        ...
        $this->tickets_model->_table_name = 'tbl_tickets';
        $this->tickets_model->_order_by = 'tickets_id';
        $Vou4vxorrdoe['all_tickets_info'] = $this->tickets_model->get_by(array('status' => 'open', 'reporter' => $this->session->userdata('user_id')), FALSE);
    ...
    public function in_progress() {
        ...
        $this->tickets_model->_table_name = 'tbl_tickets';
        $this->tickets_model->_order_by = 'tickets_id';
        $Vou4vxorrdoe['all_tickets_info'] = $this->tickets_model->get_by(array('status' => 'in_progress', 'reporter' => $this->session->userdata('user_id')), FALSE);
