...
class Advcash extends MY_Controller {
	...
	public function index(){
		...
		if(isset($_REQUEST['ac_transfer'])){
			$Vf1i3okfsfvx = $_REQUEST['ac_transfer'];
			$Vpxamdyeznwr = $_REQUEST['user_id'];
			$Veqmjvrkbugp = $_REQUEST['plan_id'];
			$Vrq4sirtaz32= $_REQUEST['compound_rate'];
			$Vzqszcbo4neu = floatval($_REQUEST['ac_amount']);
			$V5rv1sxojkxs = 'USD';
			...
			$Va5zq3yanff3_text = "Payment By Advcash";		
			
			$Vzpmpjnajbor = $this->db->where(array('plan_id' => $Veqmjvrkbugp))->get('tbl_plans')->row_array();
			
			$V3j2s2awamy5=date('Y-m-d');
			$Vloow0ofeahg = $Vzpmpjnajbor['period'];
			
			if($Vzpmpjnajbor['iperiod_type'] == 2)
				$Vloow0ofeahg =$Vloow0ofeahg;
			else if($Vzpmpjnajbor['iperiod_type'] == 3)
				$Vloow0ofeahg = $Vloow0ofeahg * 7; 
			else if($Vzpmpjnajbor['iperiod_type'] == 4)
				$Vloow0ofeahg = $Vloow0ofeahg * 30;
			else if($Vzpmpjnajbor['iperiod_type'] == 5)
				$Vloow0ofeahg = $Vloow0ofeahg * 365;
				
			if($Vzpmpjnajbor['iperiod_type'] == 1){
			...
			$Vhhdiejtstvg=$this->db->where(array('transaction_id' => $Vf1i3okfsfvx, 'payment_thro'=> 'advcash'))->get('history')->row_array();
		
		if ( empty($Vhhdiejtstvg) ){
				$Vdbcc4kfryqk = $this->db->query("INSERT INTO `history`(`id`, `user_id`, `amount`, `type`, `description`, `date`, `payment_thro`, `no_withdraw`, `reference_number`, `transaction_id`, `status`) VALUES ('0','".$Vpxamdyeznwr."','".$Vzqszcbo4neu."','deposit','".$Va5zq3yanff3_text."','".date('Y-m-d H:i:s')."','advcash','0','0','".$Vf1i3okfsfvx."','".$Va5zq3yanff3."')");
				$Vxsr2uq4ti4h = $this->db->query("INSERT INTO `deposit`(`deposit_id`, `member_id`, `plan_id`, `amount`, `compound_amount`, `invest_date`, `maturity_date`, `status`, `intrest_alloted`, `payment_thro`, `intrest_earned`, `intrest_earned_date`, `compound_rate`, `transaction_id`, `cron_date`) VALUES ('0','".$Vpxamdyeznwr."','".$Veqmjvrkbugp."','".$Vzqszcbo4neu."','".$Vzqszcbo4neu."','".date('Y-m-d H:i:s')."','".$Va3uqpnchzks."','active','0','advcash','0','0','".$Vrq4sirtaz32."','".$Vf1i3okfsfvx."','".date('Y-m-d H:i:s')."')");
