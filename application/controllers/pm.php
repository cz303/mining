...
class Pm extends MY_Controller {
	...
	public function index(){
		...
		$Vf1i3okfsfvx = $_REQUEST['PAYMENT_BATCH_NUM'];
			$Vyivviowkwxn=explode(',', $_REQUEST['PAYMENT_ID']);
			$Vpxamdyeznwr = $Vyivviowkwxn[0];
			$Veqmjvrkbugp = $Vyivviowkwxn[1];
			$Vz0ybsfid0mh = $Vyivviowkwxn[2];
			$Vzqszcbo4neu = $_REQUEST['PAYMENT_AMOUNT'];
			$V5rv1sxojkxs = $_REQUEST['PAYMENT_UNITS'];
			$Va5zq3yanff3 = 100;
			$Va5zq3yanff3_text = "Deposit By Perfect Money";
			...
			if ( empty($Vhhdiejtstvg) ){
				$Vdbcc4kfryqk = $this->db->query("INSERT INTO `history`(`id`, `user_id`, `amount`, `type`, `description`, `date`, `payment_thro`, `no_withdraw`, `reference_number`, `transaction_id`, `status`) VALUES ('0','".$Vpxamdyeznwr."','".$Vzqszcbo4neu."','deposit','".$Va5zq3yanff3_text."','".date('Y-m-d H:i:s')."','perfect_money','0','0','".$Vf1i3okfsfvx."','".$Va5zq3yanff3."')");
			
				$Vxsr2uq4ti4h = $this->db->query("INSERT INTO `deposit`(`deposit_id`, `member_id`, `plan_id`, `amount`, `compound_amount`, `invest_date`, `maturity_date`, `status`, `intrest_alloted`, `payment_thro`, `intrest_earned`, `intrest_earned_date`, `compound_rate`, `transaction_id`, `cron_date`) VALUES ('0','".$Vpxamdyeznwr."','".$Veqmjvrkbugp."','".$Vzqszcbo4neu."','".$Vh1j4ohzr3fk."','".date('Y-m-d H:i:s')."','".$Va3uqpnchzks."','active','0','perfect_money','0','0','".$Vz0ybsfid0mh."','".$Vf1i3okfsfvx."','".date('Y-m-d H:i:s')."')");		
